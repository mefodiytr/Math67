...
### Запуск в локальной сети (mikl / dev-сервер)

Если нужно запустить сайт **внутри локальной сети** (доступ с планшета / ноутбука
по `http://mikl:4321/`) — есть отдельная пошаговая шпаргалка:

📄 [`deploy/local-network.md`](deploy/local-network.md)
